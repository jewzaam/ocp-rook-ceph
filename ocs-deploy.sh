...
OCS_PER_AZ=1
MACHINESETS=$("$OC" -n openshift-machine-api get machinesets -o custom-columns=name:metadata.name,replicas:spec.replicas --no-headers  | grep 'worker' | grep -v ocs | awk '{ print $1 }')
...
echo -n "Waiting for all replicas..."
while true;
do
    COUNT=$(oc get machinesets -A -o json  | jq -r '.items[] | select(.status.replicas != .status.availableReplicas) | .metadata.name' | wc -l)
    if [ "$COUNT" != "0" ];
    then
        echo -n "."
        sleep 15
